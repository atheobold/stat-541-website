...
::: callout-check-in
3.  Here is some `dplyr` code. Re-write it in `data.table` syntax.
...
4.  Using either the `tictoc` or `microbenchmark` packages, perform a speed test
...
::: callout-check-in
5.  When data are scraped from the web, you often have repeated processes, such as scraping each of many identical pages. Why would it be a bad idea to parallelize webscraping code?
...
6.  What would you put instead of `map()` in the code below to make the
...
![](./images/penguins_sizes.png){fig-alt="A screenshot of the size of the same dataset (penguins) stored two different ways. The penguins.csv file is shown to be 14.9 KB and the penguins.parquet file is shown to be 6.3 KB."}
...
::: callout-check-in
7.  Run the following code to download a medium sized dataset from the `nycflights13` package as both a csv and parquet.
...
8.  Use profiling to compare three different methods of reading in
`flights.csv`:

- with `readr::read_csv()`, 
- with the `data.table::fread()`, and
<!-- 3.27 sec elapsed -->
- with `arrow::read_csv_arrow()`.
...
tic()
readr::read_csv(here::here("flights.csv"))
toc()
...
tic()
data.table::fread(here::here("flights.csv"))
toc()
...
tic()
arrow::read_csv_arrow(here::here("flights.csv"))
toc()
...
However, when the data are large, this approach is not recommended. The resulting file will be require a lot of storage space, which might prevent it from being accepted on GitHub.

Instead, another option is to create an `inst` folder in your package repository. This folder is meant to contain any and all items that you want to be downloaded when users download your package. Inside the `inst` folder, you can make a folder called `extdata` ("external data"). You should put any `.parquet` or `.csv` data files in this folder.
