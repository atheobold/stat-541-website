...
::: callout-check-in
1. The following code triggers an error. Oh no!
...
Use `debugonce(var)` to find **the exact line of code** in the `var()` function where the error is triggered.
:::
...
![](./images/nasser_junior-misspelled.jpeg){fig-alt="A series of four frames of a stick person debugging their code. The first frame has the person encountering an error (indicated with a red caution symbol) and gritting their teeth and saying 'I hate this job.' In the second frame, the person is still dealing with the error but is now pulling on their face and saying 'and I hate my life.' The third frame has the person visibly screaming 'Why is this happening to me,' followed by a calm and resolved person in the forth frame saying 'nevermind I misspelled a variable."}
...
Yikes! This is the wrong answer! 

What happened here is that R stores *factors* as integers:

- `1` stands in for the first category,
- `2` for the second category, and 
- `3` stands for the third category. 

It doesn't matter if those categories are "3, 4, 5" or "a, b, c", they will still be stored as numbers `1`, `2`, and `3`. So, forcing this object to be numeric doesn't "read" the category names of `"3"`, `"4"`, and `"5"` - it pulls out the index numbers `1`, `2` and `3`!
:::
...
penguins <- penguins |>
  mutate(
    species_numeric = recode_values(species, 
                                    "Adelie" ~ 1,
                                    "Chinstrap" ~ 2,
                                    "Gentoo" ~ 3
                                    )
...
If you are interested in reading more about debugging, here are a couple of 
high quality readings!

[Debugging R: The Complete Toolkit, From traceback() to RStudio Breakpoints](https://r-statistics.co/R-Debugging.html)

[Statistical Computing for R and Python: Debugging](https://srvanderplas.github.io/stat-computing-r-python/part-gen-prog/06-debugging.html)
:::
