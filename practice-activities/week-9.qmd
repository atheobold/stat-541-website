...
Begin by looking at the source code of `beetle_classify_nearest`, which is in the
`classify.R` file.  In your own words, what do the steps of this function seem to 
be doing?

Then, notice that `beetle_classify_nearest` depends on two new functions:
`beetle_species` and `beetle_centroid`.  What `.R` file are these in?  What do
they do?  What new functions to *they* depend on?  What do those do?  And so on.
...
All your unit tests should be included formally in the package using the `testthat`
structure.
